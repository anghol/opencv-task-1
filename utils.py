...
def filter_image(image: cv.Mat, filter: str, average_ksize=5, median_ksize=7,
    ...
    if filter == Filter.AVERAGE:
        ...
        if gauss_ksize % 2 != 1:
            gauss_ksize = 5
        filtered_image = cv.GaussianBlur(image, (gauss_ksize, gauss_ksize), x_deviation, y_deviation)
...
def morph_transform(image: cv.Mat, operation: str, ksize=5, iters=1):
...
def detect_edges(image: cv.Mat, algorithm: str, gauss_ksize=3, sobel_ksize=5,
